...
inline ListNode::ListNode(std::vector<std::shared_ptr<NodeBase>> nodes) : nodes(std::move(nodes))
{

}

inline int ListNode::cmp(ListNode& other)
{
	int i = 0;
...
inline NodeType ListNode::getNodeType()
{
	return NodeType::List;
}

inline std::shared_ptr<ListNode> ListNode::parse(std::string input, int& pos)
{
	if (input[pos] != '[')
	{
...
inline NodeType NumberNode::getNodeType()
{
	return NodeType::Number;
}

inline NumberNode::NumberNode(int num) :number(num)
{
